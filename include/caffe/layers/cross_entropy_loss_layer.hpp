...
/**
 * @brief Computes the cross entropy loss for a one-of-many
 *        classification task, directly taking a predicted probability
 *        distribution and probability labels as input. The main difference
 *        between this layer and the multinomial_logistic_loss layer is that
 *        this layer accepts soft labels.
 *
 ...
 *      the labels @f$ l @f$, an float-valued Blob with values
 *      @f$ l_n \in [0, K - 1] @f$
 *      indicating the correct class probability label among the @f$ K @f$ classes
  * @param top output Blob vector (length 1)
 *   -# @f$ (1 \times 1 \times 1 \times 1) @f$
 *      the computed cross-entropy loss: @f$ E =
 *        \frac{-1}{N} \sum\limits_{n=1}^N l_n \log(\hat{p}_{n,l_n})
 *      @f$
 */
...
class CrossEntropyLossLayer : public LossLayer<Dtype> {
 ...
 protected:
  ...
  /**
   * @brief Computes the cross entropy loss error gradient w.r.t. the
   *        predictions.
   *
